...
models, a `HypothesisContract`, resources, feedback, generators, and renderers.
Use action contracts for probe/diagnostic tools; submit tools are generated by
the hypothesis contract. The runtime canonicalizes raw tool arguments into
domain actions, handles budget, errors, trap state, submissions, event logs, and
replay. Surfaces compile the same IR into prompts, provider tool schemas,
